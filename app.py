...
def home():
    ...
    for item in danh_sach:
        ...
        if item.get("loai_hang") == "ThucPham" and item.get("han_su_dung"):
            try:
                hsd = datetime.strptime(item["han_su_dung"], "%Y-%m-%d").date()
    ...
    thong_ke_data = {
        "labels": [item["ten_hang"] for item in danh_sach],
        "ton_kho": [item["so_luong"] for item in danh_sach],
        "da_ban": [item["da_ban"] for item in danh_sach],
